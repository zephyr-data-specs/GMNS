# General Modeling Network Specification - Documentation
Here, you can find human-readable information on the possible values in the specification.
...
![Depicts lanes, with lane -1 to the left of the centerline, and lanes 1, 2, 3 to the right of the centerline](/docs/img/spec_figure1.png)  
_Lanes leading to an intersection. Source: MATSim documentation (2016), with text added by Volpe._

It is challenging to represent a lane that has different purposes on different sections of a lane (e.g., a merge lane downstream of a signal, which then becomes a parking lane, and then becomes a right-turn pocket). [`segment`](spec/segment.md) and [`segment_lanes`](spec/segment_lanes.md) are used to identify the places where lane transitions occur.
...
1. The action first checks if there is a difference in any file in the [`spec` folder](../spec/) from one commit to the next. If so, it proceeds.
2. After performing the necessary setup, the action automatically runs the [`generate_docs.py`](../scripts/generate_docs.py) script, which does the following:
   1. Combine schema files with relevant data from [`datapackage.json`](../spec/datapackage.json) to generate individual documentation using [Frictionless Framework](https://framework.frictionlessdata.io/), then combine everything to create `README.md`. All generated markdown files will be placed in the [`docs/spec` folder](./spec/).
   2. Remove old [`gmns.sqlite`](../usage/database/gmns.sqlite) and SQL files and use Frictionless again to generate new DB file. Query the new DB for the schema of each table to generate SQL files.
