...
2. After performing the necessary setup, the action automatically runs the [`generate_docs.py`](../scripts/generate_docs.py) script, which does the following:
   1. Combine schema files with relevant data from [`datapackage.json`](../spec/datapackage.json) to generate individual documentation using [Frictionless Framework](https://framework.frictionlessdata.io/), then combine everything to create `README.md`
